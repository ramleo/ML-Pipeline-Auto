...
# In Docker, system deps are pre-installed — skip host checks
IS_DOCKER=false
...
if ! $IS_DOCKER; then
    ...
    if [[ "$(uname)" == "Darwin" ]]; then
        ...
        # 2. libomp (required by LightGBM on macOS)
        if ! brew list libomp &>/dev/null 2>&1; then
            echo -e "${YELLOW}⚠  libomp not found — installing (required for LightGBM)...${RESET}"
            brew install libomp
        else
            echo -e "  ${GREEN}✔ libomp (OpenMP)${RESET}"
        fi
    fi
else
    echo -e "  ${GREEN}✔ Docker environment — system deps pre-installed${RESET}"
fi
...
echo "  1) Shell wizard  — guided setup here, then auto-runs the ML pipeline"
echo "  2) Python CLI    — richer prompts via init.py"
echo ""
read -rp "Enter choice [1/2] (default: 1): " ENTRY_MODE
...
while [ -z "$TARGET_COLUMN" ]; do
    read -rp "Target column name (required): " TARGET_COLUMN
    TARGET_COLUMN=$(echo "$TARGET_COLUMN" | xargs)
    if [ -z "$TARGET_COLUMN" ]; then
        echo -e "  ${YELLOW}⚠ Target column is required — please enter the column name to predict.${RESET}"
    fi
done
...
# ML_OUTPUT_DIR is set by Docker (ENV in Dockerfile) to /output.
# On host it falls back to the parent of the template directory.
OUTPUT_BASE="${ML_OUTPUT_DIR:-$(dirname "$TEMPLATE_DIR")}"
PROJECT_DIR="${OUTPUT_BASE}/${PROJECT_NAME}_${TIMESTAMP}"
...
STAGING_DIR="${OUTPUT_BASE}/.ml_staging_${PROJECT_NAME}_${TIMESTAMP}"
...
if $IS_DOCKER; then
    # Reuse packages pre-installed in the Docker image — fast, no re-download
...
echo -e "${CYAN}${BOLD}╠══════════════════════════════════════════════════╣${RESET}"
echo -e "${CYAN}${BOLD}║  ▶   Starting ML pipeline...                     ║${RESET}"
echo -e "${CYAN}${BOLD}╚══════════════════════════════════════════════════╝${RESET}"
echo ""

# ── Step 12: Run ML pipeline ───────────────────────────────────────
echo -e "${GREEN}▶ Running ML pipeline (no Claude required)...${RESET}"
cd "$PROJECT_DIR"
