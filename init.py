...
def menu(title: str, options: list, default: str = "1") -> str:
    print(f"\n{B}{title}{X}")
...
def collect_inputs() -> dict:
    ...
    while not target_column:
        target_column = input("  Target column name (required): ").strip()
        if not target_column:
            print(f"  {Y}⚠ Target column is required — enter the column name to predict.{X}")
